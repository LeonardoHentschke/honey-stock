import React, { useEffect } from 'react';
import {
  Modal,
  View,
  Text,
  TextInput,
  ScrollView,
  Pressable,
  KeyboardAvoidingView,
  Platform,
  StyleSheet,
  ActivityIndicator,
} from 'react-native';
import { Controller, useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { X } from 'lucide-react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';

import { customerSchema, type CustomerValues } from '../../models/customerSchemas';
import type { Customer } from '../../models/customerService';

interface Props {
  visible: boolean;
  editingCustomer: Customer | null;
  isSaving: boolean;
  error: string | null;
  onSave: (values: CustomerValues) => void;
  onClose: () => void;
}

export function CustomerFormSheet({ visible, editingCustomer, isSaving, error, onSave, onClose }: Props) {
  const { bottom } = useSafeAreaInsets();

  const { control, handleSubmit, watch, reset, formState: { errors } } = useForm<CustomerValues>({
    resolver: zodResolver(customerSchema),
    defaultValues: {
      name: '',
      type: 'final',
      phone: null,
      email: null,
      document: null,
      address: null,
      notes: null,
      business_name: null,
      reseller_discount_percent: null,
    },
  });

  useEffect(() => {
    if (visible) {
      reset(
        editingCustomer
          ? {
              name: editingCustomer.name,
              type: editingCustomer.type as 'final' | 'reseller',
              phone: editingCustomer.phone,
              email: editingCustomer.email,
              document: editingCustomer.document,
              address: editingCustomer.address,
              notes: editingCustomer.notes,
              business_name: editingCustomer.business_name,
              reseller_discount_percent: editingCustomer.reseller_discount_percent,
            }
          : {
              name: '',
              type: 'final',
              phone: null,
              email: null,
              document: null,
              address: null,
              notes: null,
              business_name: null,
              reseller_discount_percent: null,
            }
      );
    }
  }, [visible, editingCustomer, reset]);

  const customerType = watch('type');
  const isReseller = customerType === 'reseller';

  return (
    <Modal
      visible={visible}
      animationType="slide"
      presentationStyle="pageSheet"
      onRequestClose={onClose}
    >
      <View style={styles.container}>
        <View style={styles.header}>
          <Text style={styles.title}>
            {editingCustomer ? 'Editar cliente' : 'Novo cliente'}
          </Text>
          <Pressable onPress={onClose} hitSlop={8} style={styles.closeBtn}>
            <X size={20} color="#6B6258" />
          </Pressable>
        </View>

        <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          <ScrollView
            style={styles.flex}
            contentContainerStyle={styles.content}
            keyboardShouldPersistTaps="handled"
          >
            {/* Tipo */}
            <Controller
              control={control}
              name="type"
              render={({ field: { onChange, value } }) => (
                <View style={styles.field}>
                  <Text style={styles.label}>Tipo *</Text>
                  <View style={styles.chipRow}>
                    {([
                      { value: 'final', label: 'Cliente Final' },
                      { value: 'reseller', label: 'Revendedor' },
                    ] as const).map((opt) => (
                      <Pressable
                        key={opt.value}
                        style={[styles.chip, value === opt.value && styles.chipActive]}
                        onPress={() => onChange(opt.value)}
                      >
                        <Text style={[styles.chipText, value === opt.value && styles.chipTextActive]}>
                          {opt.label}
                        </Text>
                      </Pressable>
                    ))}
                  </View>
                </View>
              )}
            />

            {/* Nome */}
            <Controller
              control={control}
              name="name"
              render={({ field: { onChange, onBlur, value } }) => (
                <View style={styles.field}>
                  <Text style={styles.label}>Nome *</Text>
                  <TextInput
                    style={[styles.input, errors.name && styles.inputError]}
                    value={value}
                    onChangeText={onChange}
                    onBlur={onBlur}
                    placeholder="Nome completo"
                    placeholderTextColor="#A89E91"
                  />
                  {errors.name && <Text style={styles.errorText}>{errors.name.message}</Text>}
                </View>
              )}
            />

            {/* Nome fantasia (reseller only) */}
            {isReseller && (
              <Controller
                control={control}
                name="business_name"
                render={({ field: { onChange, onBlur, value } }) => (
                  <View style={styles.field}>
                    <Text style={styles.label}>Nome fantasia</Text>
                    <TextInput
                      style={styles.input}
                      value={value ?? ''}
                      onChangeText={(t) => onChange(t || null)}
                      onBlur={onBlur}
                      placeholder="Ex: Mel das Colmeias"
                      placeholderTextColor="#A89E91"
                    />
                  </View>
                )}
              />
            )}

            {/* Telefone */}
            <Controller
              control={control}
              name="phone"
              render={({ field: { onChange, onBlur, value } }) => (
                <View style={styles.field}>
                  <Text style={styles.label}>Telefone</Text>
                  <TextInput
                    style={styles.input}
                    value={value ?? ''}
                    onChangeText={(t) => onChange(t || null)}
                    onBlur={onBlur}
                    placeholder="(51) 99999-9999"
                    placeholderTextColor="#A89E91"
                    keyboardType="phone-pad"
                  />
                </View>
              )}
            />

            {/* E-mail */}
            <Controller
              control={control}
              name="email"
              render={({ field: { onChange, onBlur, value } }) => (
                <View style={styles.field}>
                  <Text style={styles.label}>E-mail</Text>
                  <TextInput
                    style={[styles.input, errors.email && styles.inputError]}
                    value={value ?? ''}
                    onChangeText={(t) => onChange(t || null)}
                    onBlur={onBlur}
                    placeholder="email@exemplo.com"
                    placeholderTextColor="#A89E91"
                    keyboardType="email-address"
                    autoCapitalize="none"
                  />
                  {errors.email && <Text style={styles.errorText}>{errors.email.message}</Text>}
                </View>
              )}
            />

            {/* Documento */}
            <Controller
              control={control}
              name="document"
              render={({ field: { onChange, onBlur, value } }) => (
                <View style={styles.field}>
                  <Text style={styles.label}>{isReseller ? 'CNPJ / CPF' : 'CPF'}</Text>
                  <TextInput
                    style={styles.input}
                    value={value ?? ''}
                    onChangeText={(t) => onChange(t || null)}
                    onBlur={onBlur}
                    placeholder={isReseller ? '00.000.000/0000-00' : '000.000.000-00'}
                    placeholderTextColor="#A89E91"
                    keyboardType="numeric"
                  />
                </View>
              )}
            />

            {/* Desconto (reseller only) */}
            {isReseller && (
              <Controller
                control={control}
                name="reseller_discount_percent"
                render={({ field: { onChange, onBlur, value } }) => (
                  <View style={styles.field}>
                    <Text style={styles.label}>Desconto padrão (%)</Text>
                    <TextInput
                      style={[styles.input, errors.reseller_discount_percent && styles.inputError]}
                      value={value != null ? String(value) : ''}
                      onChangeText={(t) => onChange(t ? parseFloat(t.replace(',', '.')) || null : null)}
                      onBlur={onBlur}
                      placeholder="Ex: 10"
                      placeholderTextColor="#A89E91"
                      keyboardType="decimal-pad"
                    />
                    {errors.reseller_discount_percent && (
                      <Text style={styles.errorText}>{errors.reseller_discount_percent.message}</Text>
                    )}
                  </View>
                )}
              />
            )}

            {/* Endereço */}
            <Controller
              control={control}
              name="address"
              render={({ field: { onChange, onBlur, value } }) => (
                <View style={styles.field}>
                  <Text style={styles.label}>Endereço</Text>
                  <TextInput
                    style={styles.input}
                    value={value ?? ''}
                    onChangeText={(t) => onChange(t || null)}
                    onBlur={onBlur}
                    placeholder="Rua, número, bairro, cidade"
                    placeholderTextColor="#A89E91"
                  />
                </View>
              )}
            />

            {/* Observações */}
            <Controller
              control={control}
              name="notes"
              render={({ field: { onChange, onBlur, value } }) => (
                <View style={styles.field}>
                  <Text style={styles.label}>Observações</Text>
                  <TextInput
                    style={[styles.input, styles.textarea]}
                    value={value ?? ''}
                    onChangeText={(t) => onChange(t || null)}
                    onBlur={onBlur}
                    placeholder="Anotações sobre o cliente..."
                    placeholderTextColor="#A89E91"
                    multiline
                    numberOfLines={3}
                    textAlignVertical="top"
                  />
                </View>
              )}
            />

            {error && <Text style={styles.submitError}>{error}</Text>}
          </ScrollView>

          <View style={[styles.footer, { paddingBottom: bottom + 16 }]}>
            <Pressable
              style={[styles.saveBtn, isSaving && styles.saveBtnDisabled]}
              onPress={handleSubmit(onSave)}
              disabled={isSaving}
            >
              {isSaving ? (
                <ActivityIndicator color="#FFFFFF" size="small" />
              ) : (
                <Text style={styles.saveBtnText}>
                  {editingCustomer ? 'Salvar alterações' : 'Criar cliente'}
                </Text>
              )}
            </Pressable>
          </View>
        </KeyboardAvoidingView>
      </View>
    </Modal>
  );
}

const styles = StyleSheet.create({
  flex: { flex: 1 },
  container: { flex: 1, backgroundColor: '#F5F1EA' },

  header: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    paddingHorizontal: 24,
    paddingVertical: 16,
    borderBottomWidth: 1,
    borderBottomColor: '#E7E2D9',
    backgroundColor: '#FFFFFF',
  },
  title: { fontSize: 17, fontWeight: '600', color: '#1F1B16' },
  closeBtn: {
    width: 32, height: 32, borderRadius: 16,
    backgroundColor: '#F5F1EA', alignItems: 'center', justifyContent: 'center',
  },

  content: { padding: 24, gap: 20, paddingBottom: 16 },
  field: { gap: 6 },
  label: { fontSize: 13, fontWeight: '500', color: '#3B342B' },
  input: {
    height: 48, borderRadius: 10, borderWidth: 1,
    borderColor: '#E7E2D9', backgroundColor: '#FFFFFF',
    paddingHorizontal: 14, fontSize: 15, color: '#1F1B16',
  },
  inputError: { borderColor: '#B3261E' },
  textarea: { height: 80, paddingTop: 12 },
  errorText: { fontSize: 12, color: '#B3261E' },
  submitError: {
    fontSize: 13, color: '#B3261E',
    backgroundColor: '#FDECEA', borderRadius: 8, padding: 12,
  },

  chipRow: { flexDirection: 'row', gap: 8 },
  chip: {
    flex: 1, paddingVertical: 10, borderRadius: 10,
    borderWidth: 1, borderColor: '#E7E2D9', backgroundColor: '#FFFFFF',
    alignItems: 'center',
  },
  chipActive: { borderColor: '#C47C0A', backgroundColor: '#FCEFC8' },
  chipText: { fontSize: 14, color: '#6B6258', fontWeight: '500' },
  chipTextActive: { color: '#9B5F0B', fontWeight: '600' },

  footer: {
    paddingHorizontal: 24, paddingTop: 12,
    backgroundColor: '#FFFFFF',
    borderTopWidth: 1, borderTopColor: '#E7E2D9',
  },
  saveBtn: {
    height: 52, borderRadius: 14, backgroundColor: '#E89B12',
    alignItems: 'center', justifyContent: 'center',
  },
  saveBtnDisabled: { opacity: 0.6 },
  saveBtnText: { fontSize: 16, fontWeight: '600', color: '#FFFFFF' },
});
